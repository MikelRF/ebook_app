...
<template>
  <div class="ebook-slide-contents">
    ...
    <scroll class="slide-search-list" :top="66" :bottom="48" ref="scroll" v-show="searchVisible">
      ...
      <div class="no-search-result" v-show="!result">
        <span class="icon-"></span>未找到相关内容....</div>
    </scroll>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      doSearch (q) {
        return Promise.all(
          this.currentBook.spine.spineItems.map(item => // 遍历所有章节
            item.load(this.currentBook.load.bind(this.currentBook)) // 加载当前书籍内容
            .then(item.find.bind(item, q)) // 找到关键字
      ...
      hideSearchPage() {
        ...
        this.searchList = null // 清除搜索结果列表
        this.searchText = '' // 清除搜索框内容
      }
    },
    mounted () {
    }
